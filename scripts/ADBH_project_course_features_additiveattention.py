...
from keras.layers import Activation, Dense, Embedding, LSTM, SimpleRNN, GRU, Masking, Bidirectional, Dropout, TimeDistributed, Flatten, Concatenate, GlobalMaxPooling2D, Attention, BatchNormalization, AdditiveAttention
...
def bidirectional_lstm_32_32(x_train, y_train, x_test, y_test, x_val, y_val, week_type, feature_types, course, num_epochs=10, experiment=''):
    ...
    # fit the model
    history = model.fit([x_train[:, :, 45:], x_train[:, 0, :45]] , y_train, validation_data=([x_val[:, :, 45:], x_val[:, 0, :45]], y_val), epochs=num_epochs, batch_size=64, verbose=1, callbacks=[model_checkpoint_callback])
    model = tf.keras.models.load_model(checkpoint_filepath)
    # evaluate the model
    y_pred = model.predict([x_test[:, :, 45:], x_test[:, 0, :45]])
    y_pred = [1 if y[0] >= 0.5 else 0 for y in y_pred]
    # evaluate the model
    model_params = {'model': 'LSTM-bi', 'epochs': num_epochs, 'batch_size': 64, 'loss': 'binary_cross_entropy'}
    scores = evaluate(None, [x_test[:, :, 45:], x_test[:, 0, :45]], y_test, week_type, feature_types, course, y_pred=y_pred, model_name="TF-LSTM-bi-32-32" , model_params=model_params)

    y_val_pred = model.predict([x_val[:, :, 45:], x_val[:, 0, :45]])
    y_val_pred = [1 if y[0] >= 0.5 else 0 for y in y_val_pred]
    val_scores = evaluate(None, [x_val[:, :, 45:], x_val[:, 0, :45]], y_val, week_type, feature_types, course, y_pred=y_val_pred, model_name="TF-LSTM-bi-32" , model_params=model_params)
...
def bidirectional_lstm_32(x_train, y_train, x_test, y_test, x_val, y_val, week_type, feature_types, course, num_epochs=10, experiment=''):
    ...
    # evaluate the model
    model_params = {'model': 'LSTM-bi', 'epochs': num_epochs, 'batch_size': 64, 'loss': 'binary_cross_entropy'}
    scores = evaluate(None, [x_test[:, :, 45:], x_test[:, 0, :45]], y_test, week_type, feature_types, course, y_pred=y_pred, model_name="TF-LSTM-bi-32" , model_params=model_params)
...
def bidirectional_lstm_64(x_train, y_train, x_test, y_test, x_val, y_val, week_type, feature_types, course, num_epochs=10, experiment=''):
    ...
    # evaluate the model
    model_params = {'model': 'LSTM-bi', 'epochs': num_epochs, 'batch_size': 64, 'loss': 'binary_cross_entropy'}
    scores = evaluate(None, [x_test[:, :, 45:], x_test[:, 0, :45]], y_test, week_type, feature_types, course, y_pred=y_pred, model_name="TF-LSTM-bi-64" , model_params=model_params)

    y_val_pred = model.predict([x_val[:, :, 45:], x_val[:, 0, :45]])
    y_val_pred = [1 if y[0] >= 0.5 else 0 for y in y_val_pred]
    val_scores = evaluate(None, [x_val[:, :, 45:], x_val[:, 0, :45]], y_val, week_type, feature_types, course, y_pred=y_val_pred, model_name="TF-LSTM-bi-64" , model_params=model_params)
    model.save(checkpoint_filepath + experiment + '_final_e')
    return history, scores, val_scores, model
...
def bidirectional_lstm_128(x_train, y_train, x_test, y_test, x_val, y_val, week_type, feature_types, course, num_epochs=10, experiment=''):
    ...
    # evaluate the model
    model_params = {'model': 'LSTM-bi', 'epochs': num_epochs, 'batch_size': 64, 'loss': 'binary_cross_entropy'}
    scores = evaluate(None, [x_test[:, :, 45:], x_test[:, 0, :45]], y_test, week_type, feature_types, course, y_pred=y_pred, model_name="TF-LSTM-bi-128" , model_params=model_params)

    y_val_pred = model.predict([x_val[:, :, 45:], x_val[:, 0, :45]])
    y_val_pred = [1 if y[0] >= 0.5 else 0 for y in y_val_pred]
    val_scores = evaluate(None, [x_val[:, :, 45:], x_val[:, 0, :45]], y_val, week_type, feature_types, course, y_pred=y_val_pred, model_name="TF-LSTM-bi-128" , model_params=model_params)
    model.save(checkpoint_filepath + experiment + '_final_e')
    return history, scores, val_scores, model
...
def load_meta_model(course_list):
    ...
    model = FastText(vector_size=39, window=3, min_count=1, sentences=course_list, epochs=15)
    return model
...
def predict_on_transfer(best_model, exp_type, percentile, name):
    ...
    for course in courses:
        ...
        adding_meta_each_week = np.repeat(meta_feature(course + description, meta_model)[:, np.newaxis, :], course_features.shape[1], axis=1).reshape((course_features.shape[1], 39))
        ...
        if exp_type == 'baseline':
            ...
            features = [meta_features[:,:,45:], meta_features[:, 0, :45]]
...
experiment = 'self_seq_attention_project_fasttext_title_desc_meta'
exp_type = experiment
...
for percentile in early_predict:
    ...
    for course in courses:
        ...
        adding_meta_each_week = np.repeat(meta_feature(title + description, meta_model)[:, np.newaxis, :], course_features.shape[1], axis=1).reshape((course_features.shape[1], 39))
        adding_meta_each_week = np.repeat(adding_meta_each_week[np.newaxis, :, :], course_features.shape[0], axis=0)
        print(adding_meta_each_week.shape)
