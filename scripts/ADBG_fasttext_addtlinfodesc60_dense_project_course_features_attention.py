...
from keras.layers import Activation, Dense, Embedding, LSTM, SimpleRNN, GRU, Masking, Bidirectional, Dropout, TimeDistributed, Flatten, Concatenate, GlobalMaxPooling2D, Attention, BatchNormalization
...
def load_meta_model(course_list):
    ...
    model = FastText(vector_size=32, window=3, min_count=1, sentences=course_list, epochs=15)
    return model
...
def predict_on_transfer(best_model, exp_type, percentile, name):
    ...
    for course in courses:
        ...
        adding_meta_each_week = np.repeat(meta_feature(course + description, meta_model)[:, np.newaxis, :], course_features.shape[1], axis=1).reshape((course_features.shape[1], 32))
...
experiment = 'same_dim_concat_norm_32_32_attention_dense_fasttext_desc60'
exp_type = experiment
