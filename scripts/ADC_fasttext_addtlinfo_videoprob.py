...
#     adding_info_about_course = pd.get_dummies(metadata[['level']])[metadata['course_id'] == course.replace('_', '-')]
#     print(adding_info_about_course.values.shape)
#     meta_info_size = adding_info_about_course.values.shape[1]
    adding_info_about_course = pd.DataFrame()
#     adding_info_about_course['active_learning'] = metadata['active_learning']
    adding_info_about_course['percent_passing_easy_fail'] = metadata['percent_passing_easy_fail']
    return adding_info_about_course.values, 1
...
def predict_on_transfer(best_model, exp_type, percentile, name):
    ...
    for course in courses:
        ...
        meta_features = course_features

        description = list(metadata[metadata['course_id'] == course.replace('_', '-')]['title'])[0]
...
#         meta_inf, meta_info_size = get_meta_info(course)
#         adding_info_about_course = np.repeat(meta_inf[:, np.newaxis, :], meta_features.shape[1], axis=1).reshape((meta_features.shape[1], meta_info_size))
...
for percentile in early_predict:
    ...
    for course in courses:
        ...
        print(course_features.shape)
#         meta_inf, meta_info_size = get_meta_info(course)
#         adding_info_about_course = np.repeat(meta_inf[:, np.newaxis, :], course_features.shape[1], axis=1).reshape((course_features.shape[1], meta_info_size))
#         print(adding_info_about_course.shape)
...
#         print(course_features.shape)
    ...
    for model in rnn_models:
        ...
        run_name = experiment + model.__name__  + "_ep" + str(percentile) + "_" + current_timestamp
        ...
        transfer_experiment_scores = pd.concat([transfer_experiment_scores, predict_on_transfer(best_model, experiment, percentile, run_name)])
