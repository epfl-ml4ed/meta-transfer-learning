...
def predict_on_transfer(best_model, exp_type, percentile, name, transfer_courses, num_meta_features, desc_type):
    week_type = 'eq_week'
    ...
    for course in courses:
        ...
        description = list(metadata[metadata['course_id'] == course.replace('_', '-')][desc_type])[0]
...
transfer_courses = ['villesafricaines_001']
...
rnn_models = ['lstm-bi-32-32-add_info_title60_ldesc60_fasttext1642434469.1828', 'lstm-bi-64-add_info_title30_sdesc30_fasttext1642438005.27095']
experiment = 'jan31_finetune_pass_all_weeks_va'
exp_type = experiment
...
for percentile in early_predict:
    if percentile == 0.4:
        ...
        desc_meta_model = load_meta_model(list(metadata[desc_type]), desc_size)
    if percentile == 0.6:
        title_size = 30
        desc_size = 30
    ...
    for model in rnn_models:
        if "lstm-bi-64" in model and percentile == 0.4:
            continue
        if "lstm-bi-32-32" in model and percentile == 0.6:
            continue
...
#             layer.trainable = False
        ...
        transfer_experiment_scores = pd.concat([transfer_experiment_scores, predict_on_transfer(model, exp_type, percentile, run_name, transfer_courses, num_meta_features, desc_type)])
