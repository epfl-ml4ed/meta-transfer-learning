...
def bidirectional_lstm_64(x_train, y_train, x_test, y_test, x_val, y_val, x_meta_train, x_meta_test, x_meta_val, week_type, feature_types, course, num_epochs=10, experiment=''):
    ...
    firstdense = Dense(1, activation='sigmoid')
    ...
    # fit the model
    history = lstm.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=num_epochs, batch_size=64, verbose=1)

    # freeze early layers
    for layer in lstm.layers:
        # selecting layer by name
        if layer.name == 'dense' or 'bidirectional' in layer.name:
            layer.trainable = False

    history2 = lstm.fit(x_meta_train, y_train, validation_data=(x_meta_val, y_val), epochs=num_epochs, batch_size=32, verbose=1, callbacks=[model_checkpoint_callback])
    ...
    return history, history2, scores, val_scores, lstm
...
experiment = 'dense_layers_freeze_fasttext_desc_add_info'
exp_type = experiment
...
for percentile in early_predict:
    ...
    for course in courses:
        ...
        x_meta_train_c = np.zeros(x_train_c.shape)
        x_train_c_subset = np.zeros(x_train_c.shape)
        x_meta_train_c[:,:,:73] = x_train_c[:, :, :73]
        x_train_c_subset[:,:,73:] = x_train_c[:, :, 73:]
        
        x_meta_test_c = np.zeros(x_test_c.shape)
        x_test_c_subset = np.zeros(x_test_c.shape)
        x_meta_test_c[:,:,:73] = x_test_c[:, :, :73]
        x_test_c_subset[:,:,73:] = x_test_c[:, :, 73:]
        
        x_meta_val_c = np.zeros(x_val_c.shape)
        x_val_c_subset = np.zeros(x_val_c.shape)
        x_meta_val_c[:,:,:73] = x_val_c[:, :, :73]
        x_val_c_subset[:,:,73:] = x_val_c[:, :, 73:]
        
        x_train.append(x_train_c_subset)
        x_test.append(x_test_c_subset)
        x_val.append(x_val_c_subset)
        y_train.append(y_train_c)
        y_test.append(y_test_c)
        y_val.append(y_val_c)
        
        x_meta_train.append(x_meta_train_c)
        x_meta_test.append(x_meta_test_c)
        x_meta_val.append(x_meta_val_c)
    ...
    for model in rnn_models:
        ...
        history, history2, scores, val_scores, best_model = model(x_train, y_train, x_test, y_test, x_val, y_val, x_meta_train, x_meta_test, x_meta_val, week_type, feature_types, course, num_epochs=epochs, experiment=experiment)
        ...
        plot_history(history2, 'run_history/' + run_name + '_meta', counter)
        numpy_loss_history = np.array(history2.history['loss'])
        np.savetxt('run_history/' + run_name + "_loss_history_meta.txt", numpy_loss_history, delimiter=",")
