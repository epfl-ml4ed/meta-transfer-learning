...
def predict_on_transfer(best_model, exp_type, percentile, name):
    ...
    for course in courses:
        ...
        baseline_features = course_features

        description = list(metadata[metadata['course_id'] == course.replace('_', '-')]['short_description'])[0]
        adding_meta_each_week = np.repeat(meta_feature(course + description, meta_model)[np.newaxis, :, :], course_features.shape[0], axis=1).reshape((course_features.shape[0], 1, 60))
        npad = ((0, 0), (0, course_features.shape[1]-1), (0, 0))
        adding_meta_each_week = np.pad(adding_meta_each_week, pad_width=npad, mode='constant', constant_values=0)
        
        course_features = np.concatenate([adding_meta_each_week, course_features], axis=2)
        print(course_features.shape)
        
        adding_info_about_course = np.repeat(get_meta_info(course)[np.newaxis, :, :], course_features.shape[0], axis=0).reshape((course_features.shape[0], 1, 13))
        npad = ((0, 0), (0, course_features.shape[1]-1), (0, 0))
        adding_info_about_course = np.pad(adding_info_about_course, pad_width=npad, mode='constant', constant_values=0)
...
rnn_mode = True
path = '../data/result/easy-fail/'
exp_type = 'add_info_desc_60_fasttext_1_week'
week_type = 'eq_week'
feature_types = [ "lalle_conati", "boroujeni_et_al", "chen_cui", "marras_et_al"]
...
for percentile in early_predict:
    ...
    for model in rnn_models:
        ...
        history, scores, val_scores, best_model = model(x_train, y_train, x_test, y_test, x_val, y_val, week_type, feature_types, course, num_epochs=epochs, experiment=exp_type)
